...
const ImageCanvas: React.FC<ImageCanvasProps> = ({ onNewAreaSelect }) => {
  ...
  useEffect(() => {
    ...
    canvas.on('mouse:down', function (opt: fabric.IEvent<MouseEvent>) {
      ...
      if (evt.altKey === true || (target === null && !isDrawingSelection)) {
        isPanning = true;
      ...
      if (isPanning) {
        ...
        if (finalWidth > 5 && finalHeight > 5) {
          const newBBox: [number, number, number, number] = [
            ...
            selectionRect.left! + finalWidth, selectionRect.top! + finalHeight,
          ];
    ...
    canvas.on('text:changed', (e: fabric.IEvent) => {
      ...
      if (changedObject && changedObject.data?.id && changedObject.data?.type === 'ocrTextBox') {
        ...
        setAreas((prevAreas: Area[]) => prevAreas.map((area: Area) =>
          area.id === id ? { ...area, translatedString: changedObject.text || '' } : area
        ));
      }
    });
    ...
    if (backgroundImageUrl) {
      fabric.Image.fromURL(backgroundImageUrl, (img) => {
        ...
        const MAX_CANVAS_WIDTH = 1200, MAX_CANVAS_HEIGHT = 800;
        const MIN_CANVAS_WIDTH = 300, MIN_CANVAS_HEIGHT = 200;
  ...
  }, [displaySize, backgroundImageUrl, areas, selectedAreaId, onNewAreaSelect, setAreas, setSelectedAreaId, setFabricCanvasInstance]);


  return (
    <div
      style={{ width: displaySize.width, height: displaySize.height, border: '1px solid #374151' }}
